...
namespace eae6320
{
	namespace Debug
	{
		namespace UI
		{
			...
			public:
				explicit Slider(
					const PixelCoordinates i_pixelCoordinates = { 0,0 }, 
					const std::string i_sliderName = "", 
					const Color i_color = { 1.0f,1.0f,1.0f }, 
					const float i_value = 0.25f,
					const float i_minValue = 0.0f, 
					const float i_maxValue = 1.0f, 
					const float i_sliderSize = 200.0f, 
					void const* i_callbackThisPointer = nullptr, 
					float(*i_callback)(void const* i_callbackthisPointer,  const float i_currentValue, const float i_minvalue, const float i_maxvalue) = nullptr, 
