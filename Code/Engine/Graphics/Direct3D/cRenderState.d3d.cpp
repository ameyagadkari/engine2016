...
void eae6320::Graphics::cRenderState::Bind() const
{
	// Alpha Transparency
	{
		...
		commonData->s_direct3dImmediateContext->OMSetBlendState( m_blendState, noBlendFactor, defaultSampleMask );
	}
	// Depth Buffering
	{
...
bool eae6320::Graphics::cRenderState::InitializeFromBits()
{
	...
	// Alpha Transparency
	{
		...
		const HRESULT result = commonData->s_direct3dDevice->CreateBlendState( &blendStateDescription, &m_blendState );
		if ( FAILED( result ) )
		{
	...
	// Depth Buffering
	{
		...
		const HRESULT result = commonData->s_direct3dDevice->CreateDepthStencilState( &depthStateDescription, &m_depthStencilState );
		if ( FAILED( result ) )
		{
			wereThereErrors = true;
	...
	// Draw Both Triangle Sides
	{
		...
		const HRESULT result = commonData->s_direct3dDevice->CreateRasterizerState( &rasterizerStateDescription, &m_rasterizerState );
		if ( FAILED( result ) )
		{
			wereThereErrors = true;
