...
namespace
{
	...
	int luaGetPlatformType(lua_State* io_luaState);
}
...
bool eae6320::AssetBuild::Initialize()
{
	...
	// Register custom functions
	{
		...
		lua_register(s_luaState, "GetPlatformType", luaGetPlatformType);
	}
...
namespace
{
	...
	{		
		...
		lua_pushstring(io_luaState, "EAE6320_PLATFORM_D3D");
		const int returnValueCount = 1;
		return returnValueCount;
#elif defined( EAE6320_PLATFORM_GL )
		lua_pushstring(io_luaState, "EAE6320_PLATFORM_GL");
		const int returnValueCount = 1;
		return returnValueCount;
#else
		std::string errorMessage;
